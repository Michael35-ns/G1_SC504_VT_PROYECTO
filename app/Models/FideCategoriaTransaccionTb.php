...
class FideCategoriaTransaccionTb extends Model
{
    ...
    public static function Mostrar_Categorias_OBJETIVOS_BY_ID_USUARIO($idUsuario)
    {
        $pdo = DB::getPdo();
        $stmt = $pdo->prepare("
            DECLARE
                CURSOR_OUT SYS_REFCURSOR;
            BEGIN
                FIDE_PROYECTO_FINAL_PKG.FIDE_CATEGORIA_TRANSACCION_SP(:P_ID_USUARIO, :CURSOR_OUT);
            END;
        ");
        $stmt->bindParam(':P_ID_USUARIO', $idUsuario);
        $stmt->bindParam(':CURSOR_OUT', $cursor, PDO::PARAM_STMT);
