...
class FideObjetivosFinancierosTb extends Model
{
    ...
    public static function contarObjetivos()
    {
        $pdo = DB::getPdo();
        $stmt = $pdo->prepare("
            DECLARE
                v_total NUMBER;
            BEGIN
                v_total := FIDE_OBJETIVOS_TB_CONTAR_OBJETIVOS_FUNC;
                :result := v_total;
            END;
        ");
    ...
    public static function contarObjetivosActivos()
    {
        $pdo = DB::getPdo();
        $stmt = $pdo->prepare("
            DECLARE
                v_total NUMBER;
            BEGIN
                v_total := FIDE_OBJETIVOS_TB_CONTAR_OBJETIVOS_ACTIVOS_FUNC;
                :result := v_total;
            END;
        ");
        $stmt->bindParam(':result', $result, PDO::PARAM_INT | PDO::PARAM_INPUT_OUTPUT, 32);
    ...
    public static function contarObjetivosInactivos()
    {
        $pdo = DB::getPdo();
        $stmt = $pdo->prepare("
            DECLARE
                v_total NUMBER;
            BEGIN
                v_total := FIDE_OBJETIVOS_TB_CONTAR_OBJETIVOS_INACTIVOS_FUNC;
                :result := v_total;
            END;
        ");
        $stmt->bindParam(':result', $result, PDO::PARAM_INT | PDO::PARAM_INPUT_OUTPUT, 32);
    ...
    public static function calcPorcentaje()
    {
        $pdo = DB::getPdo();
        $stmt = $pdo->prepare("
            DECLARE
                v_porcentaje NUMBER;
            BEGIN
                v_porcentaje := FIDE_OBJETIVOS_FINANCIEROS_CALC_PORCENTAJE_FUNC;
                :result := v_porcentaje;
            END;
        ");
        $stmt->bindParam(':result', $result, PDO::PARAM_INT | PDO::PARAM_INPUT_OUTPUT, 32);
